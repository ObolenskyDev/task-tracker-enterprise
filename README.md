...
## 💡 Особенности реализации для ревью

Этот проект — **Proof of Concept (PoC)**, демонстрирующий мои навыки интеграции инструментов:
1. **GitHub Actions:** Автоматический прогон тестов (CI) при каждом коммите гарантирует, что в ветку `main` не попадет сломанный код.
2. **SQL Analytics:** Спроектирована нормализованная БД (3NF) с защитой целостности данных (`FOREIGN KEY`, `ON DELETE CASCADE`).
3. **QA Mindset:** Код написан через призму тестируемости — логика валидации вынесена в отдельный класс `TaskValidator`, который покрыт Unit-тестами на 100%.
